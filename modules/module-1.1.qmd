...
::: {.callout-note}
## Updating the year range
The video was recorded using data downloaded through 2021. If you downloaded your data today, your last column will be more recent (e.g. `2024 [YR2024]`). Update the end column in the `pivot_longer()` call below to match the last year column in your own dataset.
:::
...
First we call `mutate()` along with [substring()](https://www.statology.org/substring-function-in-r/) to truncate the year variable to only include the first four characters of the string. Then we use `across()` inside another `mutate()` call to transform `year` and `flfp` to numeric variables.
...
wb_data <- wb_data |> # replace wb_data with a modified version of the dataframe 
  ...
  mutate(across(c("year", "flfp"), as.numeric)) # change year and flfp to numeric
...
As a final step, we can export our clean data to a new .csv file with the `write_csv()` function from `readr`. 
